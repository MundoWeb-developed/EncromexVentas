...
function calculateSum() {
    var t = 0,  // Total productos
        a = 0,  // Total impuestos
        e = 0,  // Total sin procesar
        p = 0,  // Descuentos por producto
        s_cost = parseFloat($("#shipping_cost").val()) || 0;
    ...
    // 3. Calcular descuentos por producto
    ...
    // 4. Calcular SUBTOTAL (productos - descuentos por producto)
    var subtotal = t - p;

    // 5. Calcular descuento general COMO PORCENTAJE (cambio clave)
    var invoice_discount_percent = parseFloat($("#invoice_discount").val()) || 0;
    var invoice_discount_amount = (subtotal * invoice_discount_percent) / 100;
    
    // 6. Calcular TOTAL FINAL
    var grand_total = subtotal + a + s_cost - invoice_discount_amount;

    // Actualizar campos
    $("#total_discount_ammount").val((p + invoice_discount_amount).toFixed(2));
    $("#grandTotal").val(grand_total.toFixed(2));
    $("#net_total_text").text(subtotal.toFixed(2));
    $("#n_total").val(subtotal.toFixed(2));
}
