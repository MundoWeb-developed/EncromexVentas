...
class Customer_model extends CI_Model
{
  ...
  public function getCustomerList2($postData = null)
  {
    ## Read value
    $draw = $postData['draw'];
    $start = $postData['start'];
    ...
    $columnSortOrder = $postData['order'][0]['dir']; // asc or desc
    $searchValue = $postData['search']['value']; // Search value
    $this->db->select("COUNT(id) as n_purchases, SUM(total_amount) as total_purchases, nombre_cliente, telefono_cliente, cancelado");
    $this->db->from('invoice');
    $this->db->where('cancelado', '0');
    $this->db->where('nombre_cliente!=', '');
    $this->db->like('nombre_cliente', $searchValue);
    $this->db->group_by('nombre_cliente', 'asc');
    $totalRecords = $this->db->get()->num_rows();
    $this->db->select("COUNT(id) as n_purchases, SUM(total_amount) as total_purchases, nombre_cliente, telefono_cliente, cancelado");
    $this->db->from('invoice');
    $this->db->where('cancelado', '0');
    ...
    foreach ($records as $record) {
      ...
      $data[] = array(
        'n_purchases'     => $record->n_purchases,
  ...
  public function update_customer_data($nombre_cliente, $n_nombre_cliente, $telefono_cliente)
  {

    return $this->db->set('nombre_cliente', $n_nombre_cliente)
