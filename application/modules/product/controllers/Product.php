...
class Product extends MX_Controller {
   ...
   // category part
	...
	function bdtask_category_insumo_list() {
        $data['title']      = display('manage_category');
        $data['module']     = "product";
        $data['page']       = "category_insumo_list"; 
        $data["category_list"] = $this->product_model->category_insumo_list();
        echo modules::run('template/layout', $data);
    }
      ...
      public function bdtask_category_form($id = null)
    {
      ...
      public function bdtask_category_insumo_form($id = null)
    {
        $data['title'] = display('add_category');
        #-------------------------------#
        $this->form_validation->set_rules('category_name',display('category_name'),'required|max_length[200]');
        ...
        if ($this->form_validation->run() === true) {
            ...
            #if empty $id then insert data
            if (empty($id)) {
                if ($this->product_model->create_insumo_category($postData)) {
                    #set success message
                   $this->session->set_flashdata('message', display('save_successfully'));
                } else {
                 $this->session->set_flashdata('exception', display('please_try_again'));
                ...
                if (isset($_POST['add-another'])) {
                redirect(base_url('category_insumo_form'));
                exit;
            }else{
                redirect("category_insumo_list");
            }
            } else {
                if ($this->product_model->update_insumo_category($postData)) {
                   $this->session->set_flashdata('message', display('update_successfully'));
                } else {
                  $this->session->set_flashdata('exception', display('please_try_again'));
                } 
                ...
                if(!empty($id)){
                $data['title']    = display('edit_category');
                $data['category'] = $this->product_model->single_insumo_category_data($id);  
                }
                $data['module']   = "product";  
                $data['page']     = "category_insumo_form";  
                echo Modules::run('template/layout', $data); 
	...
	public function bdtask_deleteinsumocategory($id = null) {
        if ($this->product_model->delete_insumo_category($id)) {
      $this->session->set_flashdata('message', display('delete_successfully'));
        } else {
       $this->session->set_flashdata('exception', display('please_try_again'));
        }
	...
	public function bdtask_get_insumo_data(){		
        ...
        $insumo_info = $this->product_model->get_insumo_data($insumo);
        echo json_encode($insumo_info);	
	}
	
	
	
	public function bdtask_insumo_form($id = null){
       $data['title'] = display('add_insumo');
        #-------------------------------#
        $this->form_validation->set_rules('product_name',display('product_name'),'required|max_length[200]');
        $this->form_validation->set_rules('model',display('model'),'required|max_length[200]');
        $this->form_validation->set_rules('category_id',display('category'),'required|max_length[20]');
		...
		$price_ = $this->input->post('price',TRUE);					
       ...
       foreach ($txf as $key => $value) {
        ...
        if ($this->form_validation->run() === true) {
            ...
            #if empty $id then insert data
            if (empty($id)) {
                if ($this->product_model->create_insumo($postData)) {
                          for ($i = 0, $n = count($s_id); $i < $n; $i++) {
            $supplier_price = $sup_price[$i];
            $supp_id = $s_id[$i];
            ...
            $supp_prd = array(
                'product_id'     => $product_id,
                'supplier_id'    => $supp_id,
                'supplier_price' => $price_,
                'products_model' => $product_model,
            );

            $this->db->insert('supplier_insumo', $supp_prd);
        }
                    #set success message
                   $this->session->set_flashdata('message', display('save_successfully'));
                } else {
                ...
                redirect("insumo_list");
            } else {
                if ($this->product_model->update_insumo($postData)) {
                $this->db->where('product_id', $id)
                         ->delete("supplier_insumo");
            for ($i = 0, $n = count($s_id); $i < $n; $i++) {
            $supplier_price = $sup_price[$i];
            $supp_id = $s_id[$i];
            ...
            $this->db->insert('supplier_insumo', $supp_prd);
                ...
                if(!empty($id)){
                $data['title']    = display('edit_insumo');
                $data['product']  = $this->product_model->single_insumo_data($id);  
                }
                $data['supplier'] = $this->product_model->supplier_list();
                $data['id']       =  $id;
                $data['category_list']= $this->product_model->active_insumo_category();
                $data['unit_list']= $this->product_model->active_unit();
                $data['supplier_pr'] = $this->product_model->supplier_insumo_list($id);
                $data['taxfield'] = $taxfield;
                $data['module']   = "product";  
                $data['page']     = "insumo_form";  
                echo Modules::run('template/layout', $data); 
	...
	public function bdtask_insumo_list(){
        $data['title']         = display('manage_insumo');
        $data['total_product'] = $this->db->count_all("insumo_information");
        $data['module']        = "product";
        $data['page']          = "insumo_list"; 
        echo modules::run('template/layout', $data);
    }
	...
	public function CheckInsumoList(){
        $postData = $this->input->post();
        $data = $this->product_model->getInsumoList($postData);
        echo json_encode($data);
    } 
	...
	public function bdtask_deleteinsumo($id = null) {
            $check_calculation = $this->product_model->check_product($id);
            if($check_calculation > 0){
                $this->session->set_flashdata('exception', 'You can not Delete this product because it already engaged in calculation');
                redirect("insumo_list");
            }
        if ($this->product_model->delete_insumo($id)) {
      $this->session->set_flashdata('message', display('delete_successfully'));
        } else {
       $this->session->set_flashdata('exception', display('please_try_again'));
        }
