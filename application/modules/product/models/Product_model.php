...
class Product_model extends CI_Model {
	...
	public function category_insumo_list(){
        return $this->db->select('*')
      ->from('insumo_category')
      ->get()
      ->result();
     }
	...
	public function create_insumo_category($data = [])
    {    
        return $this->db->insert('insumo_category',$data);
    }
	...
	public function update_insumo_category($data = [])
    {
        return $this->db->where('category_id',$data['category_id'])
            ->update('insumo_category',$data); 
    } 
	...
	public function single_insumo_category_data($id){
        return $this->db->select('*')
            ->from('insumo_category')
            ->where('category_id', $id)
            ->get()
	...
	public function delete_insumo_category($id){
        $this->db->where('category_id', $id)
            ->delete("insumo_category");
        if ($this->db->affected_rows()) {
            return true;
        } else {
            return false;
	...
	public function get_insumo_data($id){
		
		$this->db->select('*')
            ->from('insumo_information')
            ->where('id', $id);			
	...
	public function active_insumo_category() {
        $this->db->select('*');
        $this->db->from('insumo_category');
        $this->db->where('status', 1);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
	...
	public function insumo_product_list($id){
        $this->db->select('*');
        $this->db->from('insumo_product');
        $this->db->where('product_id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
	...
	public function supplier_insumo_list($id){
        $this->db->select('*');
	...
	public function single_insumo_data($id){
         return $this->db->select('*')
            ->from('insumo_information')
            ->where('product_id', $id)
            ->get()
	...
	public function create_insumo($data = []){
        return $this->db->insert('insumo_information',$data);
    }
	...
	public function update_insumo($data = [])
    {
        return $this->db->where('product_id',$data['product_id'])
            ->update('insumo_information',$data); 
    }
       ...
       public function getProductList($postData=null){
         ...
         if($searchValue != ''){ 
            ...
            $searchQuery = " (a.product_name like '%".$searchValue."%' or a.product_model like '%".$searchValue."%' or a.price like'%".$searchValue."%' or c.supplier_price like'%".$searchValue."%' or m.supplier_name like'%".$searchValue."%'  or d.category_name like'%".$searchValue."%' ) ";
         }
	...
	public function getInsumoList($postData=null){
         ...
         foreach($records as $record ){
          ...
          $jsaction = "return confirm('Are You Sure ?')";
           ...
           $button .= '<a href="'.$base_url.'product/product/bdtask_deleteinsumo/'.$record->product_id.'" class="btn btn-xs btn-danger "  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
         }
         ...
         //$button .='  <a href="'.$base_url.'barcode/'.$record->product_id.'" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="left" title="'.display('barcode').'"><i class="fa fa-barcode" aria-hidden="true"></i></a>';
      if($this->permission1->method('manage_product','update')->access()){
         $button .=' <a href="'.$base_url.'insumo_form/'.$record->product_id.'" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="left" title="'. display('update').'"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
     }
