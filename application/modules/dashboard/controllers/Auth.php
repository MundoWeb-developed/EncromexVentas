	...
	if ($this->session->userdata('isLogIn'))
		...
		if ( $this->form_validation->run())
		{
			...
			if($user->num_rows() > 0) {
				...
				if($user->row()->user_level=='2' ||  $user->row()->user_level=='3' || $user->row()->user_level=='4'){					
					$inicio = strtotime( "07:00" );
					$final = strtotime( "23:45" );
					$actual = time();					
				...
				if($iniciar){				
						...
						$arr_type_user = array('1'=>'Administrador', '2'=>'Vendedor', '3'=>'Florista', '4'=>'Repartidor');

						$tu = $user->row()->user_level;

						$fdid= 0;
						if($user->row()->user_level == '3'){
							$fdid=$user->row()->florist;
						}else if($user->row()->user_level == '4'){
							$fdid=$user->row()->deliveryman;
						}

						$user_level = $arr_type_user[$tu];
					...
					$this->session->set_flashdata('exception', 'Ingrese en el horario indicado');
					redirect('login');
