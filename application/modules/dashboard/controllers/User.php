...
class User extends MX_Controller {
   ...
   public function bdtask_userlist() {
	...
	{ 	
		...
		$data['sucursales'] = $this->db->get('branchoffice')->result(); // Obtener todas las sucursales para el dropdown

		$data['deliveryman']      = $this->user_model->alldeliveryman();
	  	$data['florist']      = $this->user_model->allflorist();
			
		/*-----------------------------------*/
		$this->form_validation->set_rules('firstname', display('first_name'),'required|max_length[50]');
		$this->form_validation->set_rules('lastname', display('last_name'),'required|max_length[50]');
		$this->form_validation->set_rules('branchoffice_id', 'Sucursal', 'required|numeric'); // Nueva validación
		#------------------------#
		if (!empty($id)) {   
		...
		$data['user'] = (object)$userLevelData = array(
			...
			'florist'   => $this->input->post('florist'),
			'branchoffice_id' => $this->input->post('branchoffice_id') 
		);
