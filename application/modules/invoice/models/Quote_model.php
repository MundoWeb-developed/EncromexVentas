		 	...
		 	if($delim=='1'){		
				...
				if($instore=='1'){
					...
					$datainv = array(
						...
						'total_tax'       => $this->input->post('total_tax',TRUE),
						'quote'         => $invoice_no,
						'quote_details' => (!empty($this->input->post('inva_details',TRUE))?$this->input->post('inva_details',TRUE):'Gracias por tu compra'),
						'quote_discount'=> $this->input->post('invoice_discount',TRUE),
						'total_discount'  => $this->input->post('total_discount',TRUE),
						'paid_amount'     => $this->input->post('paid_amount',TRUE),
						'due_amount'      => $this->input->post('due_amount',TRUE),
						'prevous_due'     => $this->input->post('previous',TRUE),
  ...
  $income = $withoutinventory - $this->input->post('total_tax',TRUE);
     ...
     $customerinfo = $this->db->select('*')->from('customer_information')->where('customer_id',$customer_id)->get()->row();
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            ...
            $data1 = array(
                'quote_details_id'   => $this->generator(15),
                'quote_id'           => $invoice_id,
            ...
            if (!empty($quantity)) {
                $this->db->insert('quote_details', $data1);
            }
 ...
 public function searchprod_byname($pname= null)
    ...
    public function supplier_rate($product_id) {
     ...
     public function supplier_price($product_id) {
      ...
      $price = (!empty($purchasedetails->supplier_price)?$purchasedetails->supplier_price:$supplier_product->supplier_price);
        ...
        $this->db->from('quote a');
        $this->db->join('quote_details c', 'c.quote_id = a.quote_id');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->join('product_information d', 'd.product_id = c.product_id');
        $this->db->where('a.quote_id', $invoice_id);
        $this->db->where('c.quantity >', 0);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
  ...
  $taxvar = [];
   for($i=0;$i<$num_column;$i++){
    ...
    public function bdtask_qoute_pos_print_direct($invoice_id = null){
        $invoice_detail = $this->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
