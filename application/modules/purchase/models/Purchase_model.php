...
class Purchase_model extends CI_Model {
 ...
 public function supplier_list(){
	...
	public function insumo_search_item($supplier_id, $product_name) {
      $query=$this->db->select('*')
                ->from('supplier_insumo a')
                ->join('insumo_information b','a.product_id = b.product_id')
                ->where('a.supplier_id',$supplier_id)
                ->like('b.product_model', $product_name, 'both')
                ->or_where('a.supplier_id',$supplier_id)
                ->like('b.product_name', $product_name, 'both')
	...
	public function retrieve_insumo_purchase_editdata($purchase_id) {
        ...
        $this->db->join('insumo_purchase_details b', 'b.purchase_id =a.purchase_id');
        $this->db->join('insumo_information c', 'c.product_id =b.product_id');
        $this->db->join('supplier_information d', 'd.supplier_id = a.supplier_id');
        $this->db->where('a.purchase_id', $purchase_id);
        $this->db->order_by('a.purchase_details', 'asc');
        $query = $this->db->get();
	...
	public function get_total_insumo($product_id, $supplier_id) {
        $this->db->select('SUM(a.quantity) as total_purchase,b.*');
        $this->db->from('insumo_purchase_details a');
        $this->db->join('supplier_insumo b', 'a.product_id=b.product_id');
        $this->db->where('a.product_id', $product_id);
        $this->db->where('b.supplier_id', $supplier_id);
        $total_purchase = $this->db->get()->row();
		...
		$this->db->join('insumo_information c', 'c.product_id = b.insumo_id', 'left');		
        ...
        //$available_quantity = ($total_purchase->total_purchase - $total_sale->total_sale);
		$available_quantity = ($total_purchase->total_purchase - $utilizado);
	...
	public function count_purchase_insumo() {
        $this->db->select('a.*,b.supplier_name');
        $this->db->from('insumo_purchase a');
        $this->db->join('supplier_information b', 'b.supplier_id = a.supplier_id');
        $this->db->order_by('a.purchase_date', 'desc');
	...
	public function getPurchaseList_insumo($postData=null){
         $response = array();
         $fromdate = $this->input->post('fromdate');
         $todate   = $this->input->post('todate');
         if(!empty($fromdate)){
        ...
        $this->db->join('supplier_information b', 'b.supplier_id = a.supplier_id','left');
         ...
         foreach($records as $record ){
          ...
          $jsaction = "return confirm('Are You Sure ?')";
           ...
           //$button .='  <a href="'.$base_url.'insumo_purchase_details/'.$record->purchase_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('purchase_details').'"><i class="fa fa-window-restore" aria-hidden="true"></i></a>';
      if($this->permission1->method('manage_purchase','update')->access()){
         $button .=' <a href="'.$base_url.'insumo_purchase_edit/'.$record->purchase_id.'" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="'. display('update').'"><i class="fa fa-pencil" aria-hidden="true"></i></a> ';
     }
