...
class Purchase extends MX_Controller {
	...
	function bdtask_insumo_form() {
        $data['title']      = display('add_purchase');
        $data['all_supplier']= $this->purchase_model->supplier_list();
        $data['module']     = "purchase";
        $data['page']       = "add_insumo_form"; 
        echo modules::run('template/layout', $data);
    }
	...
	public function bdtask_insumo_purchase_list(){
        $data['title']      = display('manage_purchase');
        $data['total_purhcase']= $this->purchase_model->count_purchase_insumo();
        $data['module']     = "purchase";
        $data['page']       = "purchase_insumo"; 
        echo modules::run('template/layout', $data);
    }
...
public function bdtask_purchase_edit_form($purchase_id = null){
	...
	public function bdtask_insumo_purchase_edit_form($purchase_id = null){
        $purchase_detail = $this->purchase_model->retrieve_insumo_purchase_editdata($purchase_id);
        $supplier_id = $purchase_detail[0]['supplier_id'];
        $supplier_list = $this->purchase_model->supplier_list();
        ...
        $data['module']     = "purchase";
        $data['page']       = "edit_insumo_purchase_form"; 
        echo modules::run('template/layout', $data);
}
	...
	public function CheckPurchaseList_insumo(){
        $postData = $this->input->post();
        $data = $this->purchase_model->getPurchaseList_insumo($postData);
        echo json_encode($data);
    }
	...
	public function bdtask_save_insumo(){
    $this->form_validation->set_rules('supplier_id', display('supplier') ,'required|max_length[15]');
    $this->form_validation->set_rules('paytype', display('payment_type') ,'required|max_length[20]');
    $this->form_validation->set_rules('chalan_no', display('invoice_no') ,'required|max_length[20]|is_unique[insumo_purchase.chalan_no]');
    $this->form_validation->set_rules('product_id[]',display('product'),'required|max_length[20]');
    $this->form_validation->set_rules('product_quantity[]',display('quantity'),'required|max_length[20]');
    $this->form_validation->set_rules('product_rate[]',display('rate'),'required|max_length[20]');
    ...
    if ($this->form_validation->run() === true) {
       ...
       $bankcoaid = '';
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            $product_id = $p_id[$i];
            $value = $this->insumo_supplier_check($product_id, $supplier_id);
            if ($value == 0) {
                $this->session->set_flashdata('error_message', display('product_and_supplier_did_not_match'));
                redirect(base_url('add_purchase'));
                exit();
     ...
     $bankc = array(
        ...
        $this->db->insert('insumo_purchase', $data);
        $this->db->insert('acc_transaction_insumo',$coscr);
        $this->db->insert('acc_transaction_insumo',$purchasecoatran);
        if($this->input->post('paytype') == 2){
          if(!empty($paid_amount)){
        $this->db->insert('acc_transaction_insumo',$bankc);
       
        $this->db->insert('acc_transaction_insumo',$supplierdebit);
      }
        }
        if($this->input->post('paytype') == 1){
          if(!empty($paid_amount)){
        $this->db->insert('acc_transaction_insumo',$cashinhand);
        $this->db->insert('acc_transaction_insumo',$supplierdebit); 
        }    
        }       
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            ...
            if (!empty($quantity)) {
                $this->db->insert('insumo_purchase_details', $data1);
            }
        }
     $this->session->set_flashdata('message', display('save_successfully'));
        redirect("insumo_purchase_list");
	...
	public function bdtask_insumo_update_purchase() {
    $purchase_id  = $this->input->post('purchase_id',TRUE);
    $this->form_validation->set_rules('supplier_id', display('supplier') ,'required|max_length[15]');
    $this->form_validation->set_rules('paytype', display('payment_type') ,'required|max_length[20]');
    $this->form_validation->set_rules('chalan_no', display('invoice_no') ,'required|max_length[20]');
    ...
    if ($this->form_validation->run() === true) {
       ...
       $bankc = array(
        ...
        if ($purchase_id != '') {
            $this->db->where('purchase_id', $purchase_id);
            $this->db->update('insumo_purchase', $data);
            //account transaction update
             $this->db->where('VNo', $purchase_id);
            $this->db->delete('acc_transaction_insumo');
            $this->db->where('purchase_id', $purchase_id);
            $this->db->delete('insumo_purchase_details');
        }
        ...
        if($this->input->post('paytype') == 2){
          if(!empty($paid_amount)){
        $this->db->insert('acc_transaction_insumo',$bankc);
        $this->db->insert('acc_transaction_insumo',$supplier_debit);
      }
        }
        if($this->input->post('paytype') == 1){
          if(!empty($paid_amount)){
        ...
        $this->session->set_flashdata('message', display('update_successfully'));
           redirect("insumo_purchase_list");
            ...
            $this->session->set_flashdata('exception', validation_errors());
            redirect("insumo_purchase_edit/".$purchase_id);
         } 
    }
	...
	public function bdtask_insumo_search_by_supplier() {
        $supplier_id = $this->input->post('supplier_id',TRUE);
        $product_name = $this->input->post('product_name',TRUE);
        $product_info = $this->purchase_model->insumo_search_item($supplier_id, $product_name);
        if(!empty($product_info)){
        $list[''] = '';
        foreach ($product_info as $value) {
            $json_product[] = array('label'=>$value['product_name'].'('.$value['product_model'].')','value'=>$value['product_id']);
	...
	public function bdtask_retrieve_insumo_data() {
        $product_id  = $this->input->post('product_id',TRUE);
        $supplier_id = $this->input->post('supplier_id',TRUE);
        $product_info = $this->purchase_model->get_total_insumo($product_id, $supplier_id);
	...
	public function insumo_supplier_check($product_id, $supplier_id) {
        $this->db->select('*');
        $this->db->from('supplier_insumo');
        $this->db->where('product_id', $product_id);
        $this->db->where('supplier_id', $supplier_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
